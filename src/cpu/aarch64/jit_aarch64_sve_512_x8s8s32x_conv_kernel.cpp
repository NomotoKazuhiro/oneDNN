...
#include "cpu/aarch64/jit_aarch64_sve_512_x8s8s32x_conv_kernel.hpp"
...
bool _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::maybe_eltwise(
        int position) {
...
template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::prepare_output(int ur_w) {
    int nb_oc_block
            = jcp.is_depthwise ? jcp.nb_ch_blocking : jcp.nb_oc_blocking;
    for (int k = 0; k < nb_oc_block; k++)
        for (int j = 0; j < ur_w; j++) {
            auto vmm = vmm_out(j, k);
            CGA64::eor(vmm.d, vmm.d, vmm.d);
        }
    if (!jcp.signed_input) {
        CGA64::eor(reg_scratch, reg_scratch, reg_scratch);
        if (jcp.is_depthwise && !jcp.is_fast_depthwise) {
            CGA64::mov_imm(xa::WReg(reg_tmp0_imm.getIdx()), 128);
            CGA64::dup(vmm_shift.s, xa::WReg(reg_tmp0_imm.getIdx()));
        } else {
...
template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::cvt2ps(data_type_t type_in,
        const xa::ZReg vmm_in, const xa::XReg reg_base, const int offset,
        bool mask_flag) {
    ...
    switch (type_in) {
        ...
        case data_type::s32:
            if (mask_flag)
                CGA64::ld1w(vmm.s, ktail_mask / xa::T_z, xa::ptr(reg_addr));
            else
                CGA64::ld1w(vmm.s, mask_all_one, xa::ptr(reg_addr));
            break;
        case data_type::s8:
            CGA64::sub(reg_stack, reg_stack, 64);
            CGA64::str(vmm_tmp, xa::ptr(reg_stack));
            vmm_load_src(vmm_tmp, reg_addr, mask_flag);
            CGA64::zip1(vmm_tmp.b, vmm_tmp.b, vmm_tmp.b);
            CGA64::zip1(vmm_tmp.h, vmm_tmp.h, vmm_tmp.h);
            CGA64::sxtb(vmm.s, mask_all_one / xa::T_m, vmm_tmp.s);
            if (mask_flag) {
                CGA64::not_(mask_tmp.b, mask_all_one.b, ktail_mask.b);
                CGA64::mov(vmm.s, mask_tmp / xa::T_m, 0);
            }
            CGA64::ldr(vmm_tmp, xa::ptr(reg_stack));
            CGA64::add(reg_stack, reg_stack, 64);
            break;
        case data_type::u8:
            CGA64::sub(reg_stack, reg_stack, 64);
            CGA64::str(vmm_tmp, xa::ptr(reg_stack));
            ...
            CGA64::zip1(vmm_tmp.b, vmm_tmp.b, vmm_tmp.b);
            CGA64::zip1(vmm_tmp.h, vmm_tmp.h, vmm_tmp.h);
            CGA64::uxtb(vmm.s, mask_all_one / xa::T_m, vmm_tmp.s);
            if (mask_flag) {
                CGA64::not_(mask_tmp.b, mask_all_one.b, ktail_mask.b);
                CGA64::mov(vmm.s, mask_tmp / xa::T_m, 0);
            }
    ...
    if (type_in != data_type::f32)
        CGA64::scvtf(vmm_in.s, mask_all_one,
                vmm_in.s); //< vcvtdq2ps(vmm_in, vmm_in);
}

template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::compute_eltwise(int ur_w) {
    int nb_oc_block
            = jcp.is_depthwise ? jcp.nb_ch_blocking : jcp.nb_oc_blocking;
    if (ur_w == jcp.ur_w)
...
template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::store_output(
        int ur_w, bool last_oc_block_flag) {
    int nb_oc_block
            = jcp.is_depthwise ? jcp.nb_ch_blocking : jcp.nb_oc_blocking;
    int oc_block = jcp.is_depthwise ? jcp.ch_block : jcp.oc_block;

    CGA64::ldr(reg_bias, xa::ptr(reg_param1, GET_OFF(bias)));
    CGA64::ldr(reg_ptr_scales, xa::ptr(reg_param1, GET_OFF(scales)));
    if (!jcp.signed_input)
        CGA64::ldr(
                reg_compensation, xa::ptr(reg_param1, GET_OFF(compensation)));
    ...
    for (int k = 0; k < nb_oc_block; k++) {
        ...
        if (!jcp.is_fast_depthwise && jcp.signed_input) {
            ...
            CGA64::ld1w(vmm_comp.s, mask_all_one, xa::ptr(reg_addr));
        }
        for (int j = 0; j < ur_w; j++) {
            auto vmm = vmm_out(j, k);
            if (jcp.is_fast_depthwise) {
                ...
                auto zmm_tmp1 = xa::ZReg(31);
                auto zmm_tmp2 = xa::ZReg(30); //TODO: Check index
                auto zmm_tmp3 = xa::ZReg(29);
                CGA64::sub(reg_stack, reg_stack, 64);
                CGA64::str(zmm_tmp1, xa::ptr(reg_stack));
                CGA64::sub(reg_stack, reg_stack, 64);
                CGA64::str(zmm_tmp2, xa::ptr(reg_stack));
                CGA64::sub(reg_stack, reg_stack, 64);
                CGA64::str(zmm_tmp3, xa::ptr(reg_stack));
                CGA64::mov(zmm_tmp1.s, 15);
                CGA64::and_(zmm_tmp1.b, mask_all_one, zmm_permute.b);
                for (int i = 0; i < 16; i++) {
                    CGA64::cmpeq(mask_tmp.s, mask_all_one, zmm_tmp1.s, i);
                    CGA64::dup(zmm_tmp2.s, zmm.s[i]);
                    CGA64::mov(zmm_tmp3.s, mask_tmp / xa::T_m, zmm_tmp2.s);
                }
                CGA64::mov(zmm.d, zmm_tmp3.d);
                CGA64::ldr(zmm_tmp3, xa::ptr(reg_stack));
                CGA64::add(reg_stack, reg_stack, 64);
                CGA64::ldr(zmm_tmp2, xa::ptr(reg_stack));
                CGA64::add(reg_stack, reg_stack, 64);
                CGA64::ldr(zmm_tmp1, xa::ptr(reg_stack));
                CGA64::add(reg_stack, reg_stack, 64);
            }
            // vcvtdq2ps(vmm, vmm);
            CGA64::scvtf(vmm.s, mask_all_one, vmm.s);
            if (!jcp.signed_input) CGA64::fsub(vmm.s, vmm.s, vmm_comp.s);
            if (jcp.with_bias) CGA64::fadd(vmm.s, vmm.s, vmm_bias.s);

            // const Vmm vmm_k = vmm_mask(vmm, mask_flag);
            if (!jcp.is_fast_depthwise && jcp.signed_input) {
                // vmulps(vmm_k, vmm, vmm_comp);
                CGA64::fmul(vmm.s, vmm.s, vmm_comp.s);
                if (mask_flag) {
                    CGA64::not_(mask_tmp.b, mask_all_one.b, ktail_mask.b);
                    CGA64::mov(vmm.s, mask_tmp / xa::T_m, 0);
                }
                ...
                CGA64::sub(reg_stack, reg_stack, 64);
                CGA64::str(vmm_tmp, xa::ptr(reg_stack));
                CGA64::ld1w(vmm_tmp.s, mask_all_one, xa::ptr(reg_addr));
                CGA64::fmul(vmm.s, vmm.s, vmm_tmp.s);
                CGA64::ldr(vmm_tmp, xa::ptr(reg_stack));
                CGA64::add(reg_stack, reg_stack, 64);
                if (mask_flag) {
                    CGA64::not_(mask_tmp.b, mask_all_one.b, ktail_mask.b);
        ...
        for (int k = 0; k < nb_oc_block; k++) {
            ...
            for (int j = 0; j < ur_w; j++) {
                ...
                cvt2ps(jcp.dst_dt, vmm_prev_dst, reg_out, aux_output_offset,
                        mask_flag);
                if (*p_sum_scale == 1.f) {
                    CGA64::fadd(vmm.s, vmm.s, vmm_prev_dst.s);
                } else {
                    // vfmadd231ps(vmm, vmm_prev_dst, zword_b[reg_ptr_sum_scale]);
                    CGA64::sub(reg_stack, reg_stack, 64);
                    CGA64::str(vmm_tmp, xa::ptr(reg_stack));
                    CGA64::ld1rw(vmm_tmp.s, mask_all_one / xa::T_z,
                            xa::ptr(reg_ptr_sum_scale));
                    CGA64::fmla(vmm.s, mask_all_one / xa::T_m, vmm_prev_dst.s,
                            vmm_tmp.s);
                    CGA64::ldr(vmm_tmp, xa::ptr(reg_stack));
                    CGA64::add(reg_stack, reg_stack, 64);
                }
    ...
    if (one_of(jcp.dst_dt, u8, s8, s32)) {
        ...
        CGA64::mov_imm(aux_reg_saturation, float2int(saturation_ubound));
        CGA64::dup(vmm_saturation.s, xa::WReg(aux_reg_saturation.getIdx()));
        ...
        for (int k = 0; k < nb_oc_block; k++) {
            for (int j = 0; j < ur_w; j++) {
                ...
                if (jcp.dst_dt == data_type::u8) {
                    ...
                    CGA64::fmaxnm(vmm.s, mask_all_one, vmm_zero.s);
                    CGA64::fmax(vmm.s, mask_all_one, vmm_zero.s);
                }
                // vminps(vmm, vmm, vmm_ubound);
                CGA64::fminnm(vmm.s, mask_all_one, vmm_saturation.s);
                CGA64::fmin(vmm.s, mask_all_one, vmm_saturation.s);

                CGA64::frintn(vmm.s, mask_all_one, vmm.s);
                CGA64::fcvtzs(vmm.s, mask_all_one, vmm.s);
            }
        }
#endif
    }
    ...
    for (int k = 0; k < nb_oc_block; k++) {
        ...
        for (int j = 0; j < ur_w; j++) {
            ...
            CGA64::add_imm(reg_tmp_adr, base, re, reg_tmp_imm);
            ...
            switch (jcp.dst_dt) {
                ...
                case data_type::s32:
                    ...
                    CGA64::st1w(vmm.s, _mask, xa::ptr(reg_tmp_adr));
                    break;
                // case data_type::s8: vpmovsdb(addr, r_vmm); break;
                case data_type::s8:
                    CGA64::smin(vmm.s, 127);
                    CGA64::smax(vmm.s, -128);
                    CGA64::st1b(vmm.s, _mask, xa::ptr(reg_tmp_adr));
                    break;
                // case data_type::u8: //< vpmovusdb(addr, r_vmm); break;
                case data_type::u8:
                    CGA64::umin(vmm.s, 255);
                    CGA64::st1b(vmm.s, _mask, xa::ptr(reg_tmp_adr));
                    break;
...
template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::compute_ker_dw(int ur_w,
        int pad_l, int pad_r, ic_block_t last_ic_block_flag, bool h_padded) {
    assert(!"invalid group blocking for depthwise convolution");
}

template <>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Zmm>::compute_ker_dw(int ur_w,
        int pad_l, int pad_r, ic_block_t last_ic_block_flag, bool h_padded) {
    ...
    auto compute
            = [=](xa::ZReg vreg_acc, xa::ZReg vreg_wei, xa::ZReg vreg_src) {
                  // vpdpbusd(vreg_acc, vreg_src, vreg_wei);
                  CGA64::sdot(vreg_acc.s, vreg_src.b, vreg_wei.b);
              };
    ...
    if (!jcp.signed_input) {
        CGA64::eor(zmm_shifted_zero.d, zmm_shifted_zero.d, zmm_shifted_zero.d);
        CGA64::sub(zmm_shifted_zero.b, zmm_shifted_zero.b, vmm_shift.b);
    }
        ...
        if (jcp.is_resrc_depthwise && !h_padded) {
            ...
            for (int ii = ii_start; ii <= ii_end; ii++) {
                ...
                if (jcp.is_fast_depthwise) {
                    ...
                    CGA64::ldr(
                            xa::QReg(zmm_inp_msk.getIdx()), xa::ptr(reg_addr));
                    CGA64::ptrue(mask_tmp.d, xa::VL2);
                    CGA64::splice(zmm_inp_msk.d, mask_tmp.d, zmm_inp_msk.d);
                    CGA64::ptrue(mask_tmp.d, xa::VL4);
                    CGA64::splice(zmm_inp_msk.d, mask_tmp.d, zmm_inp_msk.d);
                } else {
                    ...
                    auto reg_addr
                            = get_comp_addr_reg(aux_reg_inp, aux_input_offset);
                    auto zmm_tmp = xa::ZReg(31);
                    CGA64::sub(reg_stack, reg_stack, 64);
                    CGA64::str(zmm_tmp, xa::ptr(reg_stack));
                    if (mask_flag) {
                        CGA64::eor(mask_tmp.b, mask_all_one, mask_tmp.b,
                                mask_tmp.b);
                        CGA64::eor(mask_tmp2.b, mask_all_one, mask_tmp2.b,
                                mask_tmp2.b);
                        CGA64::uzp1(mask_tmp.h, ktail_mask.h, mask_tmp.h);
                        CGA64::uzp1(mask_tmp.b, mask_tmp.b, mask_tmp2.b);
                    } else {
                        CGA64::ptrue(mask_tmp.b, xa::VL16);
                    }
                    CGA64::ld1b(zmm_tmp.b, mask_tmp, xa::ptr(reg_addr));
                    CGA64::zip1(zmm_tmp.b, zmm_tmp.b, zmm_tmp.b);
                    CGA64::zip1(zmm_tmp.h, zmm_tmp.h, zmm_tmp.h);
                    CGA64::uxtb(
                            zmm_inp_msk.s, mask_all_one / xa::T_m, zmm_tmp.s);
                    if (mask_flag) {
                        CGA64::not_(mask_tmp.b, mask_all_one.b, ktail_mask.b);
                        CGA64::mov(zmm_inp_msk.s, mask_tmp / xa::T_m, 0);
                    }
                    CGA64::ldr(zmm_tmp, xa::ptr(reg_stack));
                    CGA64::add(reg_stack, reg_stack, 64);
                }
                if (!jcp.signed_input)
                    CGA64::sub(zmm_inp_tmp.b, zmm_inp_tmp.b, vmm_shift.b);
            }
        }
        for (int ki = 0; ki < jcp.kw; ki++) {
            ...
            if (jcp.is_fast_depthwise) {
                ...
                CGA64::ldr(xa::QReg(zmm_wei.getIdx()), xa::ptr(reg_addr));
                CGA64::ptrue(mask_tmp.d, xa::VL2);
                CGA64::splice(zmm_wei.d, mask_tmp.d, zmm_wei.d);
                CGA64::ptrue(mask_tmp.d, xa::VL4);
                CGA64::splice(zmm_wei.d, mask_tmp.d, zmm_wei.d);
                // vmovdqu8(zmm_wei | kblend_mask | T_z, zmm_wei);
                CGA64::not_(mask_tmp.b, mask_all_one, kblend_mask.b);
                CGA64::mov(zmm_wei.b, kblend_mask / xa::T_m, zmm_wei.b);
                CGA64::mov(zmm_wei.b, mask_tmp / xa::T_m, 0);
            } else {
                // vpmovsxbd(zmm_wei,
                //         SVE_compress_addr(aux_reg_ker, aux_kernel_offset));
                auto reg_addr
                        = get_comp_addr_reg(aux_reg_ker, aux_kernel_offset);
                auto zmm_tmp = xa::ZReg(30);
                CGA64::sub(reg_stack, reg_stack, 64);
                CGA64::str(zmm_tmp, xa::ptr(reg_stack));
                CGA64::ldr(xa::QReg(zmm_tmp.getIdx()), xa::ptr(reg_addr));
                CGA64::zip1(zmm_tmp.b, zmm_tmp.b, zmm_tmp.b);
                CGA64::zip1(zmm_tmp.h, zmm_tmp.h, zmm_tmp.h);
                CGA64::sxtb(zmm_wei.s, mask_all_one / xa::T_m, zmm_tmp.s);
                CGA64::ldr(zmm_tmp, xa::ptr(reg_stack));
                CGA64::add(reg_stack, reg_stack, 64);
            }
            if (h_padded) {
                    ...
                    if (oi >= oi_start && oi < oi_end) {
                        if (jcp.is_resrc_depthwise) {
                            ...
                            if (jcp.is_fast_depthwise) {
                                ...
                                CGA64::ldr(xa::QReg(r_zmm_src.getIdx()),
                                        xa::ptr(reg_addr));
                                CGA64::ptrue(mask_tmp.d, xa::VL2);
                                CGA64::splice(
                                        r_zmm_src.d, mask_tmp.d, r_zmm_src.d);
                                CGA64::ptrue(mask_tmp.d, xa::VL4);
                                CGA64::splice(
                                        r_zmm_src.d, mask_tmp.d, r_zmm_src.d);
                            } else {
                                ...
                                CGA64::str(zmm_tmp, xa::ptr(reg_stack));
                                if (mask_flag) {
                                    CGA64::eor(mask_tmp.b, mask_all_one,
                                            mask_tmp.b, mask_tmp.b);
                                    CGA64::eor(mask_tmp2.b, mask_all_one,
                                            mask_tmp2.b, mask_tmp2.b);
                                    CGA64::uzp1(mask_tmp.h, ktail_mask.h,
                                            mask_tmp.h);
                                    CGA64::uzp1(mask_tmp.b, mask_tmp.b,
                                            mask_tmp2.b);
                                } else {
                                    CGA64::ptrue(mask_tmp.b, xa::VL16);
                                }
                                CGA64::ld1b(
                                        zmm_tmp.b, mask_tmp, xa::ptr(reg_addr));
                                CGA64::zip1(zmm_tmp.b, zmm_tmp.b, zmm_tmp.b);
                                CGA64::zip1(zmm_tmp.h, zmm_tmp.h, zmm_tmp.h);
                                CGA64::uxtb(r_zmm_src.s, mask_all_one / xa::T_m,
                                        zmm_tmp.s);
                                if (mask_flag) {
                                    CGA64::not_(mask_tmp.b, mask_all_one.b,
                                            ktail_mask.b);
                                    CGA64::mov(
                                            r_zmm_src.s, mask_tmp / xa::T_m, 0);
                                }
                                CGA64::ldr(zmm_tmp, xa::ptr(reg_stack));
                                CGA64::add(reg_stack, reg_stack, 64);
                            }
                            if (!jcp.signed_input)
                                CGA64::sub(zmm_src.b, zmm_src.b, vmm_shift.b);
                        }
...
template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::compute_ker(int ur_w,
        int pad_l, int pad_r, ic_block_t last_ic_block_flag, bool h_padded) {
    if (jcp.is_depthwise)
    ...
    for (int ki = 0; ki < kw; ki++) {
        ...
        for (int ic = 0; ic < icb; ic++) {
            if (h_padded) {
                ...
                CGA64::eor(inp.d, inp.d, inp.d);
                CGA64::sub(inp.b, inp.b, vmm_shift.b);
            } else {
                    ...
                    if (jj >= jj_start && jj < jj_end) {
                        if (last_ic_block_flag == last_sp_block
                            ...
                            if ((-0x40 <= re) && (re < 0x40) && ((re % 4) == 0))
                                CGA64::ld1rw(vmm_inp(jj, nb_oc_block).s,
                                        mask_all_one,
                                        xa::ptr(base,
                                ...
                                auto reg_tmp_imm = ((jj % 4) == 0)
                                        ...
                                        : ((jj % 4) == 1) ? reg_tmp1_imm
                                                        ...
                                                        : reg_tmp3_imm;
                                CGA64::add_imm(
                                        reg_tmp_adr, base, re, reg_tmp_imm);
                                CGA64::ld1rw(vmm_inp(jj, nb_oc_block).s,
                                        mask_all_one, xa::ptr(reg_tmp_adr));
                            }
                        }
                        if (!jcp.signed_input)
                            CGA64::sub(vmm_inp(jj, nb_oc_block).b,
            ...
            for (int ii = 0; ii < nb_oc_block; ii++) {
                if (!jcp.signed_input) {
                    ...
                    auto reg_addr
                            = get_comp_addr_reg(aux_reg_ker, aux_kernel_offset);
                    CGA64::ld1w(vmm_wei.s, mask_all_one, xa::ptr(reg_addr));
                    for (int jj = _start; jj < _end; jj++) {
                    ...
                    if ((ii + 1) < nb_oc_block) {
                        ...
                        auto reg_addr = get_comp_addr_reg(
                                aux_reg_ker, aux_kernel_offset);
                        CGA64::ld1w(
                                _vmm_wei.s, mask_all_one, xa::ptr(reg_addr));
                    }
                    for (int jj = _start; jj < _end; jj++) {
...
template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::kh_loop(
        int ur_w, int pad_l, int pad_r, ic_block_t last_ic_block_flag) {
    xa::LabelAArch64 kd_label, kh_label, skip_kd_loop, skip_kh_loop;
    xa::LabelAArch64 f_overflow_label, no_f_overflow_label,
            d_h_f_overflow_label, t_overflow_label, no_t_overflow_label,
            b_overflow_label, no_b_overflow_label, back_overflow_label,
    ...
    if (jcp.ndims == 5) {
        CGA64::mov(aux_reg_ker_d, reg_ker);
        ...
        if (!jcp.signed_input) {
            ...
            CGA64::ldr(reg_ki, xa::ptr(reg_param1, GET_OFF(f_overflow)));
            CGA64::cmp(reg_ki, 0);
            CGA64::b(xa::EQ, no_f_overflow_label);
            CGA64::L_aarch64(f_overflow_label);
            {
                CGA64::mov(aux_reg_ker, aux_reg_ker_d);
                CGA64::mov_imm(reg_kj, jcp.kh);
                CGA64::L_aarch64(d_h_f_overflow_label);
                {
                    compute_ker(ur_w, pad_l, pad_r, last_ic_block_flag, true);
                    CGA64::adds_imm(aux_reg_ker, aux_reg_ker, shift_kernel_ptr,
                            reg_tmp0_imm);
                    CGA64::subs(reg_kj, reg_kj, 1);
                    CGA64::b(xa::NE, d_h_f_overflow_label);
                }
                CGA64::add_imm(aux_reg_ker_d, aux_reg_ker_d,
                        shift_kernel_ptr * jcp.kh, reg_tmp0_imm);
                CGA64::subs(reg_ki, reg_ki, 1);
                CGA64::b(xa::NE, f_overflow_label);
            }
            CGA64::L_aarch64(no_f_overflow_label);
        }

        CGA64::ldr(reg_ki, xa::ptr(reg_param1, GET_OFF(kd_padding)));
        if ((!jcp.signed_input) || (jcp.dilate_d >= jcp.id)
                || (jcp.signed_input
                        && (jcp.kd - 1) * (jcp.dilate_d + 1)
                                < nstl::max(jcp.f_pad, jcp.back_pad))) {
            CGA64::cmp(reg_ki, 0);
            CGA64::b(xa::EQ, skip_kd_loop);
        }
        ...
        if (jcp.is_fused_conv) {
            CGA64::mov(aux_reg_inp_buffer_ptr, reg_inp_buffer_ptr);
        } else {
    ...
    if (!jcp.signed_input && jcp.ndims > 3) {
        CGA64::ldr(reg_overflow, xa::ptr(reg_param1, GET_OFF(t_overflow)));
        CGA64::cmp(reg_overflow, 0);
        CGA64::b(xa::EQ, no_t_overflow_label);
        CGA64::L_aarch64(t_overflow_label);
        {
            compute_ker(ur_w, pad_l, pad_r, last_ic_block_flag, true);

            CGA64::adds_imm(
                    aux_reg_ker, aux_reg_ker, shift_kernel_ptr, reg_tmp0_imm);
            CGA64::subs(reg_overflow, reg_overflow, 1);
            CGA64::cmp(reg_overflow, 0);
            CGA64::b(xa::GT, t_overflow_label);
        }
        CGA64::L_aarch64(no_t_overflow_label);
    }
    CGA64::ldr(reg_kj, xa::ptr(reg_param1, GET_OFF(kh_padding)));
    if ((!jcp.signed_input) || (jcp.dilate_h >= jcp.ih)
            || (jcp.signed_input
                    && (jcp.kh - 1) * (jcp.dilate_h + 1)
                            < nstl::max(jcp.t_pad, jcp.b_pad))) {
        CGA64::cmp(reg_kj, 0);
        CGA64::b(xa::EQ, skip_kh_loop);
    }
    CGA64::L_aarch64(kh_label);
    {
        if (jcp.is_fused_conv) {
            CGA64::ldr(aux_reg_inp, xa::ptr(aux_reg_inp_buffer_ptr));
            CGA64::add(aux_reg_inp, aux_reg_inp, reg_inp);
        }
        ...
        CGA64::adds_imm(
                aux_reg_ker, aux_reg_ker, shift_kernel_ptr, reg_tmp0_imm);
        if (jcp.is_fused_conv) {
            CGA64::adds_imm(aux_reg_inp_buffer_ptr, aux_reg_inp_buffer_ptr,
                    sizeof(void *), reg_tmp0_imm);
        } else {
            CGA64::adds_imm(aux_reg_inp, aux_reg_inp,
    ...
    CGA64::L_aarch64(skip_kh_loop);
    if (!jcp.signed_input && jcp.ndims > 3) {
        CGA64::ldr(reg_overflow, xa::ptr(reg_param1, GET_OFF(b_overflow)));
        CGA64::cmp(reg_overflow, 0);
        CGA64::b(xa::EQ, no_b_overflow_label);
        CGA64::L_aarch64(b_overflow_label);
        {
            compute_ker(ur_w, pad_l, pad_r, last_ic_block_flag, true);
            ...
            CGA64::subs(reg_overflow, reg_overflow, 1);
            CGA64::cmp(reg_overflow, 0);
            CGA64::b(xa::GT, b_overflow_label);
        }
        CGA64::L_aarch64(no_b_overflow_label);
    }

    if (jcp.ndims == 5) {
        CGA64::adds_imm(aux_reg_inp_d, aux_reg_inp_d,
                shift_input_ptr * jcp.ih * (jcp.dilate_d + 1), reg_tmp0_imm);
        CGA64::adds_imm(aux_reg_ker_d, aux_reg_ker_d, shift_kernel_ptr * jcp.kh,
                reg_tmp0_imm);
        CGA64::subs(reg_ki, reg_ki, 1);
        CGA64::b(xa::NE, kd_label);

        CGA64::L_aarch64(skip_kd_loop);
        if (!jcp.signed_input) {
            CGA64::ldr(reg_ki, xa::ptr(reg_param1, GET_OFF(back_overflow)));
            CGA64::cmp(reg_ki, 0);
            CGA64::b(xa::EQ, no_back_overflow_label);
            CGA64::L_aarch64(back_overflow_label);
            {
                CGA64::mov(aux_reg_ker, aux_reg_ker_d);
                CGA64::mov(reg_kj, jcp.kh);
                CGA64::L_aarch64(d_h_back_overflow_label);
                {
                    compute_ker(ur_w, pad_l, pad_r, last_ic_block_flag, true);
                    CGA64::adds_imm(aux_reg_ker, aux_reg_ker, shift_kernel_ptr,
                            reg_tmp0_imm);
                    CGA64::subs(reg_kj, reg_kj, 1);
                    CGA64::b(xa::NE, d_h_back_overflow_label);
                }
                CGA64::adds_imm(aux_reg_ker_d, aux_reg_ker_d,
                        shift_kernel_ptr * jcp.kh, reg_tmp0_imm);
                CGA64::subs(reg_ki, reg_ki, 1);
                CGA64::b(xa::NE, back_overflow_label);
            }
            CGA64::L_aarch64(no_back_overflow_label);
        }
    }
}

template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::icb_loop(
        int ur_w, int pad_l, int pad_r, bool is_last_sp_block) {
    prepare_output(ur_w);

    // IC loop
    xa::LabelAArch64 icb_label;
    CGA64::mov_imm(reg_icb, jcp.nb_ic);
    CGA64::L_aarch64(icb_label);
    if (jcp.ngroups % jcp.ch_block != 0 || jcp.ic_without_padding != jcp.ic) {
        xa::LabelAArch64 common_ker, end_ker;

        if (jcp.is_depthwise)
            CGA64::cmp(reg_oc_blocks, jcp.nb_ch - jcp.nb_ch_blocking);
        else
            CGA64::cmp(reg_icb, 1); // The last IC block
        CGA64::b(xa::NE, common_ker);

        kh_loop(ur_w, pad_l, pad_r,
                is_last_sp_block ? last_sp_block : last_ic_block);
        CGA64::b(end_ker);

        CGA64::L_aarch64(common_ker);
        kh_loop(ur_w, pad_l, pad_r, no_last_block);

        CGA64::L_aarch64(end_ker);
    } else {
        kh_loop(ur_w, pad_l, pad_r, no_last_block);
    }
    // End of IC Loop
    ...
    CGA64::adds_imm(reg_inp, reg_inp, jcp.typesize_in * inp_step, reg_tmp0_imm);
    CGA64::adds_imm(reg_ker, reg_ker, jcp.typesize_in * ker_step, reg_tmp0_imm);

    CGA64::subs(reg_icb, reg_icb, 1);
    CGA64::cmp(reg_icb, 0);
    CGA64::b(xa::GT, icb_label);

    CGA64::subs_imm(reg_inp, reg_inp, jcp.typesize_in * inp_step * jcp.nb_ic,
            reg_tmp0_imm);
    CGA64::subs_imm(reg_ker, reg_ker, jcp.typesize_in * ker_step * jcp.nb_ic,
            reg_tmp0_imm);

    if (jcp.ngroups % jcp.ch_block != 0 || jcp.oc_without_padding != jcp.oc) {
        xa::LabelAArch64 common_store, end_store;

        if (jcp.is_depthwise)
            CGA64::cmp(reg_oc_blocks, jcp.nb_ch - jcp.nb_ch_blocking);
        else
            CGA64::cmp(reg_oc_blocks, jcp.nb_oc - jcp.nb_oc_blocking);

        CGA64::b(xa::NE, common_store);
...
template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::vmm_mask_all_one() {
    mask_gflag = false;
    CGA64::ptrue(mask_all_one.b, xa::VL16);
}

template <>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Ymm>::vmm_mask_all_one() {
    mask_gflag = false;
    CGA64::ptrue(mask_all_one.b, xa::VL32);
}

template <>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Zmm>::vmm_mask_all_one() {
    mask_gflag = true;
    CGA64::ptrue(mask_all_one.b);
}

template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::vmm_load_src(
        xa::ZReg src, xa::XReg reg_addr, bool mask_flag) {
    if (mask_flag) {
        CGA64::eor(mask_tmp.b, mask_all_one, mask_tmp.b, mask_tmp.b);
        CGA64::eor(mask_tmp2.b, mask_all_one, mask_tmp2.b, mask_tmp2.b);
        CGA64::uzp1(mask_tmp.h, ktail_mask.h, mask_tmp.h);
        CGA64::uzp1(mask_tmp.b, mask_tmp.b, mask_tmp2.b);
    } else {
        CGA64::ptrue(mask_tmp.b, xa::VL4);
    }
    CGA64::ld1b(src.b, mask_tmp, xa::ptr(reg_addr));
}

template <>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Ymm>::vmm_load_src(
        xa::ZReg src, xa::XReg reg_addr, bool mask_flag) {
    if (mask_flag) {
        CGA64::eor(mask_tmp.b, mask_all_one, mask_tmp.b, mask_tmp.b);
        CGA64::eor(mask_tmp2.b, mask_all_one, mask_tmp2.b, mask_tmp2.b);
        ...
        CGA64::ptrue(mask_tmp.b, xa::VL8);
    }
    CGA64::ld1b(src.b, mask_tmp, xa::ptr(reg_addr));
}

template <>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Zmm>::vmm_load_src(
        xa::ZReg src, xa::XReg reg_addr, bool mask_flag) {
    if (mask_flag) {
        CGA64::eor(mask_tmp.b, mask_all_one, mask_tmp.b, mask_tmp.b);
        CGA64::eor(mask_tmp2.b, mask_all_one, mask_tmp2.b, mask_tmp2.b);
...
template <typename Vmm>
void _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Vmm>::generate() {
    xa::LabelAArch64 permute_index_table;
    ...
    if (jcp.is_depthwise) {
        ...
        if (!jcp.is_resrc_depthwise) zmm_src = xa::ZReg(++idx);
        // if (jcp.ver != ver_vnni) zmm_tmp = xa::ZReg(++idx);
        if (jcp.is_fast_depthwise) zmm_permute = xa::ZReg(++idx);
        if (!jcp.signed_input) zmm_shifted_zero = xa::ZReg(++idx);
    ...
    if (jcp.is_fused_conv) {
        CGA64::ldr(reg_inp_buffer_ptr, xa::ptr(reg_param1, GET_OFF(src)));
        ...
        Traverse the data as
            ...
            ... process row2 ...
        */
        CGA64::mov_imm(reg_inp, 0);
    } else {
        CGA64::ldr(reg_inp, xa::ptr(reg_param1, GET_OFF(src)));
    }
    CGA64::ldr(reg_out, xa::ptr(reg_param1, GET_OFF(dst)));
    CGA64::ldr(reg_ker, xa::ptr(reg_param1, GET_OFF(filt)));
    ...
    if (jcp.ngroups % jcp.ch_block != 0 || jcp.oc_without_padding != jcp.oc) {
        ...
        CGA64::ldr(reg_oc_blocks, xa::ptr(reg_param1, GET_OFF(oc_blocks)));
        // Reg32 regw_tmp = reg_oi.cvt32();
        auto regw_tmp = reg_oi;
        // mov(regw_tmp, mask);
        CGA64::mov(regw_tmp, mask);
        // kmovw(ktail_mask, regw_tmp);
        auto vmm_tmp1 = xa::ZReg(31);
        auto vmm_tmp2 = xa::ZReg(30);
        CGA64::index(vmm_tmp1.s, 0, 1);
        CGA64::mov(vmm_tmp2.s, 1);
        CGA64::lsl(vmm_tmp2.s, mask_all_one / xa::T_m, vmm_tmp1.s);
        CGA64::dup(vmm_tmp1.s, xa::WReg(regw_tmp.getIdx()));
        CGA64::and_(vmm_tmp1.d, vmm_tmp1.d, vmm_tmp2.d);
        CGA64::cmpne(ktail_mask.s, mask_all_one, vmm_tmp1.s, 0);
    }
    if (jcp.is_fast_depthwise) {
        // prepare mask register for blending weights
        // mov(reg_scratch, 0x8888444422221111);
        CGA64::movk(reg_scratch, uint16_t(0x1111), 0);
        CGA64::movk(reg_scratch, uint16_t(0x2222), 16);
        CGA64::movk(reg_scratch, uint16_t(0x4444), 32);
        CGA64::movk(reg_scratch, uint16_t(0x8888), 48);
        // kmovq(kblend_mask, reg_scratch);
        CGA64::sub(reg_stack, reg_stack, 8);
        CGA64::str(reg_scratch, xa::ptr(reg_stack));
        CGA64::ldr(kblend_mask, xa::ptr(reg_stack));
        CGA64::add(reg_stack, reg_stack, 8);
        // load permute indices from data section
        // mov(reg_scratch, permute_index_table);
        CGA64::adr(reg_scratch, permute_index_table);
        // vmovdqu32(zmm_permute, ptr[reg_scratch]);
        CGA64::ld1w(zmm_permute.s, mask_all_one, xa::ptr(reg_scratch));
    }
    ...
    if (jcp.nb_ow == 1) {
        ...
        CGA64::eor(reg_oi, reg_oi, reg_oi);
        if (jcp.ow == jcp.ur_w) {
            icb_loop(jcp.ur_w, jcp.l_pad, r_pad, true);
        } else {
            if (n_oi == 0) {
                icb_loop(jcp.ur_w, jcp.l_pad, r_pad1, jcp.ur_w_tail == 0);
                CGA64::adds_imm(reg_inp, reg_inp, inp_shift_pad, reg_tmp0_imm);
                CGA64::adds_imm(reg_out, reg_out, out_shift, reg_tmp0_imm);
                if (jcp.ur_w_tail != 0) {
                ...
                if (jcp.l_pad > 0) {
                    ...
                    CGA64::adds_imm(
                            reg_inp, reg_inp, inp_shift_pad, reg_tmp0_imm);
                    CGA64::adds_imm(reg_out, reg_out, out_shift, reg_tmp0_imm);

                    CGA64::adds(reg_oi, reg_oi, 1);
                }
                if ((jcp.l_pad <= 0 && n_oi > 0)
                    ...
                    CGA64::L_aarch64(ow_loop_label);
                    {
                        ...
                        CGA64::adds_imm(
                                reg_inp, reg_inp, inp_shift, reg_tmp0_imm);
                        CGA64::adds_imm(
                                reg_out, reg_out, out_shift, reg_tmp0_imm);
                ...
                if (r_pad1 > 0 || jcp.ur_w_tail == 0) {
                    ...
                    CGA64::adds_imm(reg_inp, reg_inp, inp_shift, reg_tmp0_imm);
                    CGA64::adds_imm(reg_out, reg_out, out_shift, reg_tmp0_imm);
                }
                if (jcp.ur_w_tail != 0) {
                    icb_loop(jcp.ur_w_tail, 0, r_pad, true);
        ...
        xa::LabelAArch64 end_label, last_oi_label, middle_ow_blocks_label,
                tail_label, oi_loop_label, oi_loop_end_label;
        ...
        CGA64::ldr(reg_owb, xa::ptr(reg_param1, GET_OFF(owb)));
        ...
        CGA64::mov_imm(reg_oi, n_oi_first_ow_block);
        if (jcp.l_pad > 0) {
            icb_loop(jcp.ur_w, jcp.l_pad, 0, false);
            CGA64::adds_imm(reg_inp, reg_inp, inp_shift_pad, reg_tmp0_imm);
            CGA64::adds_imm(reg_out, reg_out, out_shift, reg_tmp0_imm);
        ...
        if (n_oi_last_ow_block != n_oi_not_last_ow_block) {
            ...
            CGA64::mov_imm(reg_oi, n_oi_last_ow_block);
        ...
        if (n_oi_next_last_ow_block != n_oi_not_last_ow_block) {
            ...
            CGA64::mov_imm(reg_oi, n_oi_next_last_ow_block);
            CGA64::b(xa::EQ, oi_loop_label);
        }
        CGA64::mov_imm(
    ...
    if (jcp.is_fast_depthwise) {
        align(64);
        CGA64::L_aarch64(permute_index_table);
        const uint32_t _idx[]
...
bool jit_aarch64_sve_512_x8s8s32x_fwd_kernel::post_ops_ok(
...
status_t jit_aarch64_sve_512_x8s8s32x_fwd_kernel::init_conf(
...
void jit_aarch64_sve_512_x8s8s32x_fwd_kernel::init_scratchpad(
        memory_tracking::registrar_t &scratchpad, const jit_conv_conf_t &jcp,
        const primitive_attr_t &attr) {}

template struct _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Zmm>;
template struct _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Ymm>;
template struct _jit_aarch64_sve_512_x8s8s32x_fwd_kernel<Xmm>;
} // namespace aarch64
} // namespace cpu
} // namespace impl
} // namespace dnnl
